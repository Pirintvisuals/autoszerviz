...
function step(myId) {
  ...
  // Dividers are scenery, not speech — drop them in with no typing beat.
  if (msg.r === 'divider') {
    typingFor = null; revealed++
    render(); setTimeout(scrollBottom, 20)
    setTimeout(() => step(myId), 500)
    return
  }
...
function buildMessageNode(msg, i) {
  ...
  // Divider — lets one demo show a SECOND short flow after the estimate (e.g.
  // the same agent handling a vague symptom, where it refuses to price and
  // books a diagnostic instead). Purely visual, no avatar, no typing beat.
  if (msg.r === 'divider') {
    const wrap = document.createElement('div')
    wrap.style.cssText = 'display:flex;align-items:center;gap:10px;margin:18px 2px 10px;'
    wrap.innerHTML = `
      <div style="flex:1;height:1px;background:var(--line)"></div>
      <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.1em;color:#9ca3af;text-align:center">${esc(msg.t)}</div>
      <div style="flex:1;height:1px;background:var(--line)"></div>`
    return wrap
  }
...
function buildCostMessage() {
  ...
  bubble.innerHTML = `
    ...
    ${EST.note ? `<div style="font-size:11px;color:#6b7280;font-style:italic;margin-top:8px;line-height:1.4">${esc(EST.note)}</div>` : ''}`
...
function buildModal() {
  ...
  // ── Optional blocks ──────────────────────────────────────────────────────
  // Three panels of the owner card are only right for *travelling* trades who
  // quote against a customer budget. A destination trade (a szerviz, a garage:
  // the customer drives to THEM) omits the fields below and gets a panel that
  // answers the question they actually ask before picking up the phone.
  //
  //   EST.distanceMi omitted → EST.urgency  {label, value, note}
  //   EST.budget     omitted → EST.decision {label, value, note}   e.g. parts grade
  //   EST.score      omitted → EST.jobValue {label, value, note}, or nothing
  //
  // Every existing demo sets all three, so nothing about them changes.
  const distanceBlock = EST.distanceMi != null
    ...
    : EST.urgency
    ? `<div style="text-align:right">
         <div style="font-size:9.5px;color:#9ca3af;text-transform:uppercase;letter-spacing:.1em;font-weight:600">${esc(EST.urgency.label || L.requestedTiming)}</div>
         <div style="font-size:15px;font-weight:800;color:var(--ink-soft);letter-spacing:-0.3px;line-height:1.3">${esc(EST.urgency.value)}</div>
         ${EST.urgency.note ? `<div style="font-size:10px;color:#9ca3af;margin-top:2px">${esc(EST.urgency.note)}</div>` : ''}
       </div>`
    : ''

  const rightOfQuote = EST.budget
    ? `<div style="text-align:right">
    ...
    ? `<div style="text-align:right;max-width:46%">
         <div style="font-size:9.5px;color:#9ca3af;text-transform:uppercase;letter-spacing:.1em;font-weight:600;margin-bottom:3px">${esc(EST.decision.label)}</div>
         <div style="font-size:14px;font-weight:800;color:#111827;line-height:1.3">${esc(EST.decision.value)}</div>
         ${EST.decision.note ? `<div style="font-size:10.5px;color:#6b7280;margin-top:3px;line-height:1.35">${esc(EST.decision.note)}</div>` : ''}
       </div>`
    : ''

  const footBlock = EST.score != null
    ...
    ? `<div style="padding:12px 16px;background:var(--chat-bg)">
         <div style="display:flex;align-items:center;justify-content:space-between;gap:10px">
           <span style="font-size:10px;color:#6b7280;text-transform:uppercase;letter-spacing:.12em;font-weight:600">${esc(EST.jobValue.label || L.jobValue)}</span>
           <span style="font-size:15px;font-weight:800;color:var(--ink-soft);letter-spacing:-0.3px;text-align:right">${esc(EST.jobValue.value)}</span>
         </div>
         ${EST.jobValue.note ? `<div style="font-size:11px;color:#6b7280;margin-top:5px;line-height:1.4">${esc(EST.jobValue.note)}</div>` : ''}
       </div>`
    : ''

  const budgetBox = !EST.budget ? '' : budgetMatch
    ...
    <div style="padding:18px 18px 6px">
      <div style="background:var(--chat-bg);border:1.5px solid var(--line);border-radius:16px;overflow:hidden">
        ...
        <div style="padding:12px 16px;border-bottom:1px solid var(--line);background:#fff">
          ...
          ${EST.distanceMi != null ? `<div style="margin-top:10px;height:5px;background:var(--line);border-radius:3px;overflow:hidden">
